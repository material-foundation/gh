...
Repo.prototype.run = function () {
    ...
    if (options.whoami) {
        logger.log(options.user + "/" + options.repo);
    }
    ...
    if (options.new) {
        hooks.invoke('repo.new', instance, function (afterHooksCallback) {
            ...
            instance.new(function (err1, repo) {
                if (err1) {
                    logger.error(JSON.parse(err1.message).errors);
    ...
    if (options.edit) {
        hooks.invoke('repo.edit', instance, function (afterHooksCallback) {
            options.repo = options.edit;

            if (options.organization) {
                options.user = options.organization;
            }

            logger.log('Editing repo ' + logger.colors.green(options.user + '/' + options.edit));

            instance.edit(function (err1, repo) {
                if (err1) {
                    logger.error(JSON.parse(err1.message).errors);
                    logger.error('Can\'t edit repo. ' + JSON.parse(err1.message).message);
                    return;
                }
...
Repo.prototype.edit = function (opt_callback) {
    var instance = this,
        options = instance.options,
        payload,
        method = 'edit';
    ...
    if ('defaultbranch' in options) {
      payload['defaultbranch'] = options.defaultbranch;
    }
    ...
    if ('haswiki' in options) {
      payload['has_wiki'] = options.haswiki;
    }
